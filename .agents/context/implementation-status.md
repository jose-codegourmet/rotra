...
| Track | Routes | Status |
|-------|--------|--------|
| **Mock prototypes** | `/sessions/join`, `/joined`, `/queue`, `/court`, `/attendance` | MOCK — `MOCK_SESSION_*`, local toasts |
...
| `/sessions/join` `/joined` `/queue` `/court` `/attendance` | MOCK | Theatre only |
