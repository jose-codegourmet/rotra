  ...
  render: () => (
    <div className="font-sans space-y-1 p-8">
      {typeScale.map(({ token, size, weight, lh, ls }) => (
        <div key={token} className="flex items-baseline gap-8 py-3 border-b border-border last:border-0">
          ...
          <p className={`text-text-primary text-${token as any} leading-none`}>{SAMPLE}</p>
        </div>
      ))}
    </div>
  ),
  ...
  render: () => (
    <div className="font-sans space-y-1 p-8">
      {weights.map(({ value, label }) => (
        <div key={value} className="flex items-baseline gap-8 py-3 border-b border-border last:border-0">
          <div className="w-28 shrink-0 text-small text-text-secondary">
            {label} <span className="text-text-disabled">{value}</span>
          </div>
          <p className="text-body text-text-primary" style={{ fontWeight: value }}>
            {SAMPLE}
          </p>
        </div>
      ))}
    ...
    <div className="font-sans space-y-1 p-8">
      {weights.map(({ value, label }) => (
        <div key={value} className="flex items-baseline gap-8 py-3 border-b border-border last:border-0">
          ...
          <p className="text-body text-text-primary italic" style={{ fontWeight: value }}>
            {SAMPLE}
          </p>
        </div>
      ))}
