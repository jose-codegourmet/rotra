...
const PROFILE = {
  ...
  gear: {
    rackets: [{ title: 'Main Racket', brand: 'Yonex', model: 'Astrox 99', specs: ['Head Heavy', 'BG80', '26 lbs'] }],
    shoes: [],
    bags: [],
  },
}
          ...
          <div className="flex flex-col items-center gap-3 lg:items-start">
            <div className="relative">
              <div className="size-[72px] rounded-full bg-bg-elevated border-2 border-bg-elevated flex items-center justify-center">
                <span className="text-title font-black text-text-secondary">AS</span>
              </div>
            </div>
            <button type="button" className="text-small text-accent font-medium">
              Edit Photo
            </button>

            <div className="flex items-center gap-2 mt-1">
              <h2 className="text-display font-bold text-text-primary">{PROFILE.name}</h2>
              <button
                type="button"
            ...
            <div className="flex items-center gap-2 px-4 py-2 bg-bg-surface border border-border rounded-full">
              ...
              <span className="text-body font-medium text-text-primary">{PROFILE.tier}</span>
              <span className="text-small text-text-secondary">· {PROFILE.exp} EXP</span>
              <a href="/profile/exp-ledger" className="text-small text-accent font-bold ml-2">
                EXP LOG →
              </a>
            </div>
          </div>
          ...
          <div className="flex flex-col gap-4">
            ...
            <div className="flex flex-col gap-2">
              ...
              {PROFILE.gear.rackets.map((racket) => (
                ...
                >
                  <p className="text-body font-bold text-text-primary">{racket.title}</p>
            ...
            <div className="flex flex-col gap-2">
              ...
              <div className="border border-dashed border-border rounded-lg p-4 flex items-center justify-center">
                <p className="text-body text-text-disabled">+ Add your first shoes</p>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="flex flex-col gap-6">
          ...
          <div className="flex flex-col gap-3">
            ...
            <p className="text-small text-text-secondary">
              Set your baseline until enough match data is available. This phases out as ratings
              come in.
            </p>
            {['Attack', 'Defense', 'Net & Touch', 'Precision & Control', 'Athleticism', 'Game Intelligence'].map(
              (dimension) => (
                <div key={dimension} className="flex items-center justify-between gap-4">
                  <span className="text-body text-text-primary w-40 shrink-0">{dimension}</span>
                    ...
                    {[1, 2, 3, 4, 5].map((dot) => (
                      <button
                        ...
                        className={`size-4 rounded-full transition-colors duration-default ${
                          dot <= 3 ? 'bg-accent' : 'bg-bg-elevated border border-border'
                        }`}
          ...
          <div className="bg-bg-surface border border-border rounded-lg p-5 shadow-card flex flex-col gap-3">
            ...
            <p className="text-small text-text-disabled">
              Ratings are calculated from match results. You need at least 5 matches per
              dimension for a full breakdown.
            </p>
          </div>
          ...
          <div className="bg-bg-surface border border-border rounded-lg p-5 shadow-card flex flex-col gap-3">
            ...
            <p className="text-body text-text-secondary">
              Reviews from opponents are aggregated here. Individual reviewers remain anonymous.
            </p>
            <button
              type="button"
          ...
          <div className="flex flex-col gap-3">
            ...
            <div className="bg-bg-surface border border-border rounded-lg shadow-card overflow-hidden">
              <div className="flex items-center justify-center py-10">
                <p className="text-small text-text-disabled">No matches recorded yet.</p>
              </div>
            </div>
          </div>
        </div>
