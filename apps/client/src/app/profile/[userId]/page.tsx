        ...
        <div className="flex flex-col gap-6 lg:sticky lg:top-4 lg:self-start">
          ...
          <div className="flex flex-col items-center gap-3 lg:items-start">
            <div className="size-[72px] lg:size-20 rounded-full bg-bg-elevated border-2 border-bg-elevated flex items-center justify-center">
              <span className="text-title font-black text-text-secondary">{player.initials}</span>
            </div>

            <h1 className="text-display font-bold text-text-primary">{player.name}</h1>
            ...
            <div className="flex items-center gap-2 px-4 py-2 bg-bg-surface border border-border rounded-full">
              ...
              <span className="text-body font-medium text-text-primary">{player.tier}</span>
              <span className="text-small text-text-secondary">· {player.exp} EXP</span>
            </div>
          </div>
            ...
            {player.gear.map((category) => (
              <div key={category.category} className="flex flex-col gap-2">
                ...
                {category.items.length > 0 ? (
                  ...
                  <p className="text-body text-text-disabled">No gear listed.</p>
                )}
              </div>
            ))}
          </div>
              ...
              {player.recentMatches.map((match, idx) => (
                <div
                  ...
                  className={`flex items-center gap-4 px-4 py-3 ${
                    idx < player.recentMatches.length - 1 ? 'border-b border-border' : ''
                  }`}
                >
                  <span
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-small text-text-primary truncate">{match.teams}</p>
                    <p className="text-small text-text-secondary">
                      {match.date} · {match.club}
                    </p>
                  </div>
                  <span className="text-small text-text-secondary shrink-0">{match.score}</span>
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="bg-bg-surface border border-border rounded-lg p-5 shadow-card flex flex-col gap-2">
            ...
            {[
              ...
              <div key={row.label} className="flex items-center justify-between py-1">
                ...
                <span className="text-body font-medium text-text-primary">{row.value}</span>
              </div>
            ))}
          </div>
        </div>
