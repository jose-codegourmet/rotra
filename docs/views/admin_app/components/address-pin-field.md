...
A compact map picker with geocoding search that lets admins set a physical venue location by typing an address or placing/dragging a pin on a mini map.

**Admin path:** `apps/admin/src/components/ui/address-pin-field/AddressPinField.tsx`

**Map style:** Uses `ADMIN_MAP_STYLE` (`mapbox://styles/mapbox/light-v11`) from `apps/admin/src/constants/mapbox.ts` — a neutral light style suitable for data management. Do not use the client app's dark branded style.
...
Requires `NEXT_PUBLIC_MAPBOX_TOKEN` in `apps/admin/.env.local` (same token as the client app).
